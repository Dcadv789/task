...
const TaskCard: React.FC<TaskCardProps> = ({ task }) => {
  const { toggleTaskCompletion, deleteTask, clients, lists } = useAppContext();
  ...
  return (
    <>
      <div className={`bg-white rounded-lg shadow-sm border overflow-hidden transition-all duration-200 mb-4 ${
        task.completed ? 'border-gray-200' : 'border-gray-200 hover:border-blue-200'
      }`}>
        <div className="p-6">
      ...
      {showRemindersModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <TaskReminders task={task} onClose={() => setShowRemindersModal(false)} />
          </div>
        </div>
      )}
    </>
